...
def a_star(matrix, start, end, heuristic):
    ...
    open = PriorityQueue()
    open.put((globals()[heuristic](start, end),
             globals()[heuristic](start, end), start))
    aPath = {}
        ...
        for d in 'ESNW':
            if maze_map[currCell][d] == True:
                ...
                temp_f_score = temp_g_score + \
                    globals()[heuristic](childCell, end)

                if temp_f_score < f_score[childCell]:
                    g_score[childCell] = temp_g_score
                    f_score[childCell] = temp_f_score
                    open.put((temp_f_score, globals()[heuristic](
                        childCell, end), childCell))
...
if __name__ == '__main__':
    ...
    route = [end, *fwdPath][::-1]
    t = timeit(stmt='a_star(matrix, start, end, heuristic)',
