...
if __name__ == '__main__':
    ...
    fwdPath, aPath = bfs(matrix, start, end)
    route = [end, *fwdPath][::-1]
    t = timeit(stmt='bfs(matrix, start, end)', number=1000, globals=globals())
    print(f'BFS time: {t}')
    print(f'BFS search length: {len(aPath)}')
    print(f'BFS path length: {len(fwdPath)}')
